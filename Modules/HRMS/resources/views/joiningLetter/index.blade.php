            ...
            <div class="joining-generator-section">
                <form id="joiningLetterForm">
                    ...
                    <div class="joining-generator-form">
                        ...
                        <button type="button" class="joining-btn-primary" onclick="previewAndSaveLetter()">Preview
    ...
    `);
        ...
        $(document).ready(function() {
            ...
            $('#searchInput').on('keyup', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            data.forEach(emp => {
                                let fullName = emp.first_name && emp.last_name ? emp
                                    .first_name + ' ' + emp.last_name : emp.name;
                                let job = emp.job_profile || emp.department || 'N/A';
                                let status = emp.status || '⏳ Pending';
                ...
                <div class="search-item"
                    ...
                    <small>(${job})</small>
                    <span style="float:right;font-size:12px;color:#666;">${status}</span>
                </div>
            `;
                            });
                        } else {
            ...
            $(document).on('click', '.search-item', function() {
                ...
                // ✅ Update status dynamically with color
                $('#profileStatus')
                    .text(status)
                    .css({
                        color: status.toLowerCase().includes('approved') ? '#28a745' : status
                            .toLowerCase().includes('rejected') ? '#dc3545' : '#ffc107'
                    });
